...
import './SavedAnswersPage.css';
import AnswerScreen from '../AnswerScreen/AnswerScreen';

const SavedAnswersPage = ({ history, setHistory, isStarred, setIsStarred}) => {
  ...
  return (
    showAnswer ?
    <AnswerScreen text="חזור למועדפים" resetSolveScreen={() => setShowAnswer(false)} 
    ...
    <Container>
            ...
            <tbody>
              {filteredHistory.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="right-align">
                    <div onClick={() => handleNameOnclick(item.id)}>{item.name} </div>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      >
        <div className="modal-text">?האם להסיר ממועדפים</div>
